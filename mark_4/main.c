...
const char *memory_sem = "/memory-semaphore";
sem_t *memory;
...
void initSemaphores() {
    ...
    if((memory = sem_open(memory_sem, O_CREAT, 0666, 1)) == SEM_FAILED) {
        perror("sem_open: Can't create printData semaphore");
        exit(-1);
    };
}
